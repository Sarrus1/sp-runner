...
onmessage = function (event) {
  Module.FS.writeFile("plugin.smx", new Uint8Array(event.data), {
    encoding: "binary",
  });
  ...
  var spRuntime = Module._environment_new_runtime(
    spEnv,
    strFilename,
    strError,
    errorLen
  ...
  if (spRuntime === 0) {
    Module.printErr(
      "Failed to create SourcePawn runtime: " + Module.UTF8ToString(strError)
    );
    return;
  }
  ...
  Module.print(
    "natives do not behave identically to how they would in SourceMod."
  );
  Module.print("");
